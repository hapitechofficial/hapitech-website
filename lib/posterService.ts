...
export const generatePoster = async (params: PosterGenerationParams): Promise<string> => {
  ...
  try {
    ...
    // Use gemini-2.0-flash model which is stable and reliable for image generation
    ...
    if (!result.candidates || result.candidates.length === 0) {
      ...
      throw new Error('No image was generated. Please try again with different content.');
    }
